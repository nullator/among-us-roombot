...
// subscribe - выполняется запрос к БД, получается список активных хостов,
// генерируются кнопки с никами. При нажатии на кнопку - подписка на хостера.
// Подписка на хостера - из БД загружается модель юзера в которой есть поле Hosters []User.
// В это поле добавляется новый хостер. После этого модель юзера сохраняется в БД.
// Из БД загружается модель хостера, в которой есть поле Followers []User.
// В это поле добавляется новый подписчик. После этого модель хостера сохраняется в БД.
// Выодится сообщение о подписке.
